...
def demo_qr_generation():
    def generate_qr_image(self, metadata):
        ...
        Args:
            metadata: AssignmentMetadata object
        ...
        try:
            ...
            json_data = metadata.to_json() if hasattr(metadata, 'to_json') else str(metadata)
    ...
    try:
        ...
        print(
            f"🔄 Metadata round-trip test: {'✅ PASSED' if restored_metadata.assignment_id == metadata.assignment_id else '❌ FAILED'}")
        ...
        for size in test_sizes:
            try:
                qr_img = generator.generate_qr_code(
                    metadata, size=size, include_label=False)
                size_test_path = output_dir / \
                    f"{metadata.assignment_id}_{size[0]}x{size[1]}.png"
                qr_img.save(size_test_path)
                print(
                    f"   {size[0]}x{size[1]}: ✅ Generated (actual: {qr_img.size})")
            except Exception as e:
